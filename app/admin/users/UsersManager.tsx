...
export default function UsersManager() {
  ...
  const approveUser = async (userId: string) => {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(data.error || 'Unable to approve this user.');
      setItems((prev) =>
          ...
          <Table className="divide-y divide-slate-100 text-left text-sm text-slate-600">
            <TableHeader className="bg-white">
              <TableRow>
                <TableCell
                  isHeader
                  className="px-6 py-3 font-semibold uppercase tracking-widest text-xs text-slate-500 min-w-50"
                >
                  Name
                </TableCell>
                <TableCell
                  isHeader
                  className="px-6 py-3 font-semibold uppercase tracking-widest text-xs text-slate-500"
                >
                  Firm & Contact
                </TableCell>
                <TableCell
                  isHeader
                  className="px-6 py-3 font-semibold uppercase tracking-widest text-xs text-slate-500 min-w-[200px]"
                >
                  Location
                </TableCell>
                <TableCell
                  isHeader
                  className="px-6 py-3 font-semibold uppercase tracking-widest text-xs text-slate-500 min-w-[170px]"
                >
                  Visiting Card
                </TableCell>
                <TableCell
            ...
            <TableBody className="divide-y divide-slate-100 bg-white">
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="px-6 py-8"></TableCell>
                </TableRow>
              ) : (
                items.map((user) => {
                  ...
                  const locationLine = [user.address?.city, user.address?.state]
                    .filter(Boolean)
                    .join(', ');
                  const streetLine = [user.address?.line1, user.address?.line2]
                    .filter(Boolean)
                    .join(', ');
